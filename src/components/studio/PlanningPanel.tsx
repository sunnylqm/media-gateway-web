...
export function PlanningPanel({
  ...
  const pollingEnabled =
    ...
    !state.error &&
    !state.mutation &&
    state.tasks.some(activePlanningTask);
  useEffect(() => {
    if (!pollingEnabled) return;
    let stopped = false;
    let timer: ReturnType<typeof setTimeout>;
    // Schedule after the read settles, even when React batches busy updates.
    // A new task state or account cleanup stops this loop before another read.
    const tick = async () => {
  ...
  const submissionUnavailable =
    ...
    !(
      state.pending &&
      ['network', 'rate'].includes(state.error ?? '') &&
      active &&
      !blocked &&
      !state.busy &&
      !state.mutation &&
  ...
  async function adopt() {
    ...
    setAcceptConsent(null);
    const receipt = await controller.accept(
      selected.id,
      project.version,
      confirmed,
    );
  ...
  return (
    <section
      className="studio-planning"
      aria-labelledby="studio-planning-title"
    >
      ...
      {state.info?.enabled && !supportedPlanning(state.info) && (
        <p>{t('unsupported')}</p>
      )}
      {supportedPlanning(state.info) && (
        <div className="studio-plan-confirm">
          ...
          <label className="studio-plan-consent">
            <input
              ...
              onChange={(event) =>
                setConsent(event.target.checked ? consentKey : null)
              }
            />
          ...
          <button
            ...
            disabled={
              submissionUnavailable || !canStart || consent !== consentKey
            }
            onClick={() => void generate()}
          >
            {t(state.pending ? 'recover' : 'generate')}
          </button>
        </div>
      )}
      ...
      {selected && (
        <div className="studio-plan-detail">
          <p role="status">
            <b>{t(`state_${selected.state}`)}</b> ·{' '}
            {t('version', { version: selected.input_version })}
          </p>
          {activePlanningTask(selected) && <p>{t('queueHint')}</p>}
          {selected.state !== 'accepted' &&
            selected.input_version !== project.version && (
              <p className="studio-notice">{t('stale')}</p>
            )}
          {selected.state === 'accepted' && (
            <div className="studio-notice" role="status">
              <p>
                {t('acceptedVersion', {
                  version: selected.accepted_version ?? '',
                })}
              </p>
              <button
                type="button"
          ...
          {['failed', 'interrupted', 'stale', 'canceled'].includes(
            selected.state,
          ) && <p>{t('taskFailure')}</p>}
          {selected.error_code && (
            <p className="studio-note">
              {t('code', { code: selected.error_code })}
            </p>
          )}
          <p className="studio-note">
            {selected.usage.known
              ? t('usage', {
                  input: selected.usage.prompt_tokens,
                  output: selected.usage.completion_tokens,
                })
              : t('unknownUsage')}
          </p>
          ...
          {plan && canAcceptPlan(selected, project.version) && (
            <div className="studio-plan-confirm">
              <label className="studio-plan-consent">
                <input
                  type="checkbox"
                  checked={acceptConsent === acceptKey}
                  disabled={unavailable}
                  onChange={(event) =>
                    setAcceptConsent(event.target.checked ? acceptKey : null)
                  }
                />
