...
export const zh: Record<MessageKey, string> = {
  ...
  'admin.navDatabaseBackup': '数据库备份',
  'databaseBackup.title': '数据库备份',
  'databaseBackup.note':
    '定时备份 SQLite 数据库（所有账户、密钥、价格与任务记录都在其中），每份副本还可以上传到 S3 兼容存储。',
  'databaseBackup.loading': '正在加载备份配置…',
  'databaseBackup.schedule': '备份计划',
  'databaseBackup.scheduleEvery': '启动时及每 {hours} 小时备份一次',
  'databaseBackup.scheduleOff':
    '未开启：设置 GATEWAY_BACKUP_DIR 或开启 S3 上传',
  'databaseBackup.localDirectory': '本地副本保存在 {path}，保留最新 {keep} 份',
  'databaseBackup.noLocalDirectory':
    '不保留本地副本（未设置 GATEWAY_BACKUP_DIR）',
  'databaseBackup.running': '备份中…',
  'databaseBackup.lastSuccess': '上次备份 {date}，{size}：{location}',
  'databaseBackup.runNow': '立即备份',
  'databaseBackup.s3Section': '上传到 S3',
  'databaseBackup.enabled': '每份副本都上传到 S3',
  'databaseBackup.prefix': '对象前缀',
  'databaseBackup.prefixNote':
    '副本保存为 <前缀>gateway-<时间>.db.gz。填 "." 表示存放在 bucket 根目录。',
  'databaseBackup.keep': 'S3 保留份数',
  'databaseBackup.keepNote': '每次上传后，删除该前缀下超出份数的旧副本。',
  'databaseBackup.restoreNote':
    '保存前会检查 bucket 是否可访问，保存后立即上传一份。副本经过 gzip 压缩，但网关不做加密，请使用私有 bucket。恢复时下载副本、gunzip 解压后按恢复流程操作。',
  'databaseBackup.save': '保存备份配置',
  'databaseBackup.saved': '备份配置已保存。',
  'databaseBackup.savedUploading': '备份配置已保存，首次上传已开始。',
  'databaseBackup.remoteCopies': 'S3 中的副本（{count}）',
  'databaseBackup.noRemoteCopies': '还没有上传过副本。',
  'databaseBackup.columnTime': '上传时间',
  'databaseBackup.columnSize': '大小',
  'databaseBackup.columnKey': '对象键',
  'databaseBackup.errorLoad': '无法加载备份配置',
  'databaseBackup.errorSave': '无法保存备份配置',
  'databaseBackup.errorRun': '无法开始备份',
  'databaseBackup.errorKeep': 'S3 保留份数必须在 1 到 1000 之间',
  'admin.title': '系统控制',
  ...
  'storage.usageTitle': '本地磁盘',
  'storage.usageAssets': '资产占用 {size} · {count} 个对象',
  'storage.usageDisk': '磁盘已用 {used} / 共 {total}',
  'storage.usageFree': '剩余 {free}',
  'storage.usageLegendAssets': '资产',
  'storage.usageLegendOther': '其他文件',
  'storage.usageLow': '磁盘剩余空间不足 10%。',
  'storage.backupSection': '备份',
  ...
  'storage.backupStatus': '复制状态',
  'storage.backupStats': '已复制 {replicated} · 失败 {failed} · 排队 {pending}',
  'storage.backupSyncing': '同步中…',
  'storage.backupLastSync': '上次全量同步 {date}，复制 {count} 个',
  'storage.backupNeverSynced': '尚未完成过全量同步',
  'storage.backupLastError': '最近错误（{date}）：{error}',
  'storage.backupSync': '立即同步',
  'storage.backupSaveFirst': '请先保存备份配置再同步。',
  'storage.errorSync': '无法启动备份同步',
  ...
  'models.testBinding': '测试连接',
  'models.testing': '测试中…',
  'models.testBindingNote':
    '只发送上游模型名、不带提示词，不会生成内容，也不消耗 token。使用这里填写的密钥，留空则用已保存的密钥。',
  'models.testOk': '连通正常：密钥有效，上游按预期拒绝了空请求',
  'models.testRateLimited': '已连通，但上游正在限流这个密钥',
  'models.testAuthFailed': '上游拒绝了这个密钥',
  'models.testNotFound': '接口或模型不存在：请检查 API 基础地址和上游模型名',
  'models.testUpstreamError': '上游返回了服务端错误',
  'models.testUnreachable': '无法连接上游',
  'models.testAccepted':
    '上游接受了不带提示词的请求，这个地址可能不是该协议的接口',
  'models.testFailed': '测试没能执行',
  ...
  'topup.dialogNote':
    '通过 Stripe Checkout 使用银行卡付款，支付完成后余额会立即更新。',
  'topup.presetLabel': '选择金额',
  'topup.customLabel': '自定义金额',
  'topup.customPlaceholder': '请输入金额',
  'topup.customRange': '可填 {min} 至 {max}',
  'topup.selectedAmount': '本次支付 {amount}',
  'topup.errorInvalid': '请输入最多两位小数的金额',
  'topup.errorBelowMin': '单次充值不得低于 {min}',
  'topup.errorAboveMax': '单次充值不得高于 {max}',
  'topup.errorCreate': '无法发起支付',
  'topup.submit': '前往支付',
  ...
  'topupAdmin.note': '配置租户控制台自助充值的预设金额与限额。',
  'topupAdmin.enabled': '在租户控制台开放充值',
  'topupAdmin.currency': '币种',
  'topupAdmin.currencyNote': '三位 ISO 币种代码，例如 CNY 或 USD。',
  'topupAdmin.amounts': '预设金额',
  'topupAdmin.amountsNote':
    '以元为单位，用逗号分隔，最多 20 个；重复值会被去除并自动升序排列。',
  'topupAdmin.amountsPreview': '预设',
  'topupAdmin.customAmount': '允许自定义金额',
  'topupAdmin.minAmount': '最小金额',
  'topupAdmin.maxAmount': '最大金额',
  'topupAdmin.limitsNote': '以元为单位，所有预设金额都必须落在该区间内。',
  'topupAdmin.save': '保存充值设置',
  'topupAdmin.saving': '保存中…',
  'topupAdmin.saved': '充值设置已保存',
  'topupAdmin.loading': '正在加载充值设置…',
  'topupAdmin.errorLoad': '无法加载充值设置',
  'topupAdmin.errorSave': '无法保存充值设置',
  'topupAdmin.errorAmounts': '预设金额必须为正数，且最多两位小数',
  'topupAdmin.errorAmountsEmpty': '请至少填写一个预设金额',
  'topupAdmin.errorAmountsTooMany': '预设金额最多 20 个',
  'topupAdmin.errorMin': '最小与最大金额必须为正数',
  ...
  'topupAdmin.configured': '已配置',
  'topupAdmin.missing': '未设置',
  'topupAdmin.stripeHint':
    '在网关设置 STRIPE_SECRET_KEY 之前，充值功能不会开放。',
  'topupAdmin.webhookURL': 'Stripe Webhook 地址',
  'topupAdmin.webhookEvents':
    '在 Stripe 中将 Webhook 指向该地址，并订阅 checkout.session.completed 与 checkout.session.async_payment_succeeded 事件。',
  'topupAdmin.userHistory': 'Stripe 充值记录',
  'topupAdmin.userHistoryNote': '该账号发起的银行卡支付。',
  'topupAdmin.columnTime': '时间',
  'topupAdmin.columnAmount': '金额',
  'topupAdmin.columnStatus': '状态',

  'admin.navTopupOrders': '充值订单',
  'topupOrders.title': '充值订单',
  'topupOrders.note': '租户控制台发起的全部 Stripe 充值订单。',
  'topupOrders.filterLabel': '状态',
  'topupOrders.filterAll': '全部状态',
  'topupOrders.summary': '共 {count} 笔',
  'topupOrders.columnTime': '创建时间',
  'topupOrders.columnUser': '用户',
  ...
  'topupOrders.emptyNote': '当前筛选条件下还没有记录。',
  'topupOrders.errorLoad': '无法加载充值订单',
  ...
  'admin.navErrorLogs': '异常日志',
  'errorLogs.title': '异常日志',
  'errorLogs.note':
    '网关记录的警告与错误，最新的在前。用错误响应里的请求 ID 搜索，可以定位失败原因。',
  'errorLogs.last24h': '最近 24 小时',
  'errorLogs.countError': '{count} 条错误',
  'errorLogs.countWarn': '{count} 条警告',
  'errorLogs.levelLabel': '级别',
  'errorLogs.levelAll': '全部级别',
  'errorLogs.levelError': '错误',
  'errorLogs.levelWarn': '警告',
  'errorLogs.searchLabel': '搜索',
  'errorLogs.searchPlaceholder': '消息、请求 ID、代码位置或字段',
  'errorLogs.search': '搜索',
  'errorLogs.refresh': '刷新',
  'errorLogs.clear': '全部清空',
  'errorLogs.columnTime': '时间',
  'errorLogs.columnLevel': '级别',
  'errorLogs.columnMessage': '消息',
  'errorLogs.columnRequest': '请求 ID',
  'errorLogs.loading': '正在加载异常日志…',
  'errorLogs.empty': '暂无异常日志',
  'errorLogs.emptyNote': '当前筛选条件下没有记录。',
  'errorLogs.errorLoad': '无法加载异常日志',
  'errorLogs.loadMore': '加载更多',
  'errorLogs.source': '代码位置',
  'errorLogs.fields': '字段',
  'errorLogs.stack': '调用栈',
  'errorLogs.noFields': '没有结构化字段。',
  'errorLogs.filterRequest': '只看这个请求',
  'errorLogs.clearTitle': '清空全部异常日志？',
  'errorLogs.clearDescription':
