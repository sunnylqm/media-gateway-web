...
function validTicket(v: unknown, project: string): v is ProductionTicket {
  ...
  if (v.kind === 'confirm')
    return (
      validID(b.quote_id) &&
      validAmount(b.approved_max_amount) &&
      b.confirm_generation === true &&
  ...
  if (
    ...
    !validID(b.model) ||
    !isObject(b.parameters) ||
    Object.keys(b).length !== 4
  )
    return false;
  return (
    Object.entries(b.parameters).length <= 32 &&
    Object.entries(b.parameters).every(
      ([name, value]) =>
        /^[a-z][a-z0-9_]{0,63}$/.test(name) &&
        !['duration', 'constructor', 'prototype'].includes(name) &&
        !/prompt|text|url|token|key|password/i.test(name) &&
  ...
  read(): ProductionMemory {
    ...
    try {
      ...
      if (
        !isObject(v) ||
        v.version !== 1 ||
        (v.quoteID !== undefined && !validID(v.quoteID)) ||
        (v.pending !== undefined && !validTicket(v.pending, this.project))
      )
        throw new Error();
      return v as ProductionMemory;
    } catch {
      throw new Error('production_storage');
    }
  }
