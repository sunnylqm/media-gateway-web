...
export function createProductionClient(request: StudioTransport) {
  const get = (path: string, signal?: AbortSignal) =>
    request<unknown>(path, { signal, cache: 'no-store' });
  const post = (
    path: string,
    body: object,
    key: string,
  ...
  return {
    info: async (signal?: AbortSignal) =>
      readExecution(await get(`${root}/execution`, signal)),
    ...
    quote: async (body: QuoteInput, key: string, signal?: AbortSignal) => {
      const q = readQuote(
        await post(`${root}/quotes`, body, key, signal),
        body.project_id,
      );
      if (q.project_version !== body.expected_version || q.model !== body.model)
        return protocolFailure();
      return q;
    },
    getQuote: async (id: string, project: string, signal?: AbortSignal) => {
      const q = readQuote(
        await get(`${root}/quotes/${segment(id)}`, signal),
        project,
      );
    ...
    confirm: async (
      ...
      const run = readRun(
        await post(`${root}/runs`, body, key, signal),
        project,
      );
      if (
        run.quote_id !== body.quote_id ||
        run.project_version !== body.expected_version ||
        run.max_amount !== body.approved_max_amount
      )
        return protocolFailure();
      return run;
    },
    getRun: async (id: string, project: string, signal?: AbortSignal) => {
      const run = readRun(
        await get(`${root}/runs/${segment(id)}`, signal),
        project,
      );
    ...
    artifacts: async (generation: string, signal?: AbortSignal) => {
      const v = await get(
        `/v1/generations/${segment(generation)}/artifacts`,
        signal,
      );
      if (!isObject(v) || !Array.isArray(v.data) || v.data.length > 16)
        return protocolFailure();
      return v.data.filter(
        (a): a is ShotArtifact =>
          isObject(a) &&
          typeof a.id === 'string' &&
          a.generation_id === generation &&
