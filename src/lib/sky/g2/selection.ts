...
export type Choice =
  | { readonly version: 'g1' }
  | { readonly version: 'g2'; readonly recipe: Recipe };
...
export function select(
  ...
  if (seed !== undefined)
    return { version: 'g2', recipe: generate(seed, family) };
  if (
    /^g1-[0-9a-f]{8}$/i.test(params.get('seed') ?? '') ||
    family ||
    params.get('engine') === 'g1'
  )
    return { version: 'g1' };
  let next = random() >>> 0;
  ...
  try {
    ...
    /* A private-mode storage failure cannot prevent rendering. */
  }
  const options = FAMILIES.filter((f) => f !== previousFamily);
  const selected =
    options[Math.floor(stream(next ^ 0x54fc3109)() * options.length)];
  const recipe = generate(next, selected);
  try {
    storage?.setItem(HISTORY, `${seedToken(next)}:${selected}`);
  } catch {
    /* Only cross-reload non-repetition is lost. */
  }
