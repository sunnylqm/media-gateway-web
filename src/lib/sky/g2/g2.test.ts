...
describe('G2 jewel skies', () => {
  ...
  test('parses only versioned bounded seeds, with legacy routing', () => {
    ...
    for (const url of [
      ...
      '?sky=halo',
      '?engine=g1',
      '?sky=veil&seed=g1-f00dcafe',
    ])
      expect(select(url).version).toBe('g1');
  ...
  test('enforces composition and energy bounds across 4096 recipes', () => {
    ...
    for (const family of FAMILIES)
      for (let i = 0; i < 1024; i++) {
        ...
        expect(recipe.bloom).toBeLessThanOrEqual(0.2);
  ...
  test('maps OKLCH to bounded linear sRGB and preserves neutral luminance', () => {
    const gray = oklchLinear(0.5, 0, 20);
    for (const channel of gray) expect(channel).toBeCloseTo(0.125, 6);
    for (let h = 0; h < 360; h += 5)
      for (const l of [0.3, 0.5, 0.75, 0.9]) {
        for (const channel of oklchLinear(l, 0.35, h)) {
  ...
  test('keeps camera position and velocity continuous across the loop', () => {
    for (const family of FAMILIES) {
      const r = generate(42, family),
        at = (t: number) => Array.from(cameraAt(t, r).origin);
      const start = at(0),
        end = at(r.period),
        fourth = at(4);
      ...
      for (let i = 0; i < 3; i++)
        expect((start[i] - at(r.period - 0.001)[i]) / 0.001).toBeCloseTo(
          (at(0.001)[i] - start[i]) / 0.001,
          2,
        );
    }
  });
  test('caps memory and actually lowers the volume budget on slow devices', () => {
    for (const [w, h] of [
      ...
      const high = frameSize(w, h, 3),
        low = frameSize(w, h, 3, 0.5);
      ...
      expect(low.cloudWidth * low.cloudHeight).toBeLessThan(
        high.cloudWidth * high.cloudHeight,
      );
    }
  });
  test('fresh loads avoid consecutive repeats and forced links ignore storage', () => {
    ...
    const first = select('', storage, () => 42),
      second = select('', storage, () => 42);
    expect(first.version).toBe('g2');
    expect(second.version).toBe('g2');
    if (first.version !== 'g2' || second.version !== 'g2')
      throw new Error('Expected G2');
    ...
    expect(
      select('', { getItem: blocked, setItem: blocked }, () => 42).version,
    ).toBe('g2');
    expect(
      select(
        '?seed=g2-0000002a',
    ...
    ).toEqual(select('?seed=g2-0000002a'));
  });
});
