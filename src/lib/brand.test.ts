...
describe('mypub.ai brand contract', () => {
  test('keeps the two approved Chinese lines in their distinct roles', () => {
    ...
    expect(brandZh['home.title']).toBe('把你喜欢的故事，酿成短片。');
    expect(brandZh['auth.storyTitle']).toBe(brandZh['home.title']);
    expect(brandZh['app.title']).toBe(`mypub.ai — ${brandZh['home.title']}`);
    expect(brandEn['home.title']).toBe(
      'Brew a story you love into a short film.',
    );
    expect(brandEn['auth.storyTitle']).toBe(brandEn['home.title']);
  });
