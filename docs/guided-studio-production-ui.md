...
Explicit gateway refusals such as insufficient funds, expired quote or changed project do not count as successful batches. A quote-used/idempotency conflict is reconciled by reads rather than assumed unsubmitted. A refusal of a recovery attempt (including authorization or rate limits) never erases an unresolved original confirmation: only a validated receipt can acknowledge that ticket. Configuration and actual expiry are ultimately enforced by the server; the browser clock is only an early guard.
